...
const IS_DEVELOPMENT = import.meta.env?.MODE === 'development' || true;
...
const config = {
  ...
  backendLogEndpoint: 'http://localhost:8000/frontend-logs',  // Updated with full URL
  logToBackend: true
};
...
export const logger = {
  ...
  dev: async (message, ...args) => {
    if (IS_DEVELOPMENT) {
      await log(LOG_LEVELS.DEBUG, 'color: #8b5cf6;', `[DEV] ${message}`, ...args);
    }
  }
};
